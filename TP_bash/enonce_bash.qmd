...
*Cette section s'adresse aux étudiants ayant terminé en avance. Elle mobilise uniquement les commandes vues aujourd'hui, combinées de manière plus poussée.*

### Défi A : Identification du top 1 des attaquants (Pipeline strict)
En reprenant `exercice_3/logs/connexions.log`, écrivez une commande unique qui affiche **uniquement le nom d'utilisateur** (sans le préfixe `user=` et sans le décompte) ayant cumulé le plus grand nombre d'échecs de connexion (`FAILED`).

* *Indice :* chaînez votre pipeline précédent avec un nouveau `sort` numérique, `tail` (ou `head`), et un dernier `cut`.

### Défi B : Filtrage temporel avec Expressions Régulières (`grep -E`)
Affichez uniquement les lignes de connexion enregistrées entre 08h20 et 08h39 inclus.

* *Notion :* l'option `-E` de `grep` active les expressions régulières étendues. Le crochet `[23]` permet de cibler le chiffre 2 ou 3.

* *Indice :* cherchez le motif correspondant à l'intervalle horaire dans la chaîne de date.

### Défi C : Détection de fichiers suspects par date (`find`)
Dans le dossier `defi/`, utilisez la commande `find` avec l'option `-mmin` pour localiser automatiquement les fichiers créés ou modifiés au cours des 60 dernières minutes.